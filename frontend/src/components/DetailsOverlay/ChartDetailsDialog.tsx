...
export function ChartDetailsDialog({
  ...
  return (
    <Dialog>
      <DialogTrigger className="pb-10" asChild>{trigger}</DialogTrigger>
      ...
      >
        ...
        {/* Tabs */}
        <Tabs defaultValue="summary" className="w-full h-full mt-4 overflow-y-auto" dir={i18n.dir(i18n.language)}>
          <TabsList className="grid grid-cols-3 w-full h-fit mb-4">
            <TabsTrigger value="summary" style={{ fontSize: `${15 * textScalar}px` }}>{t("er.detailsDialog.summary")}</TabsTrigger>
            <TabsTrigger value="data" style={{ fontSize: `${15 * textScalar}px` }}>{t("er.detailsDialog.data")}</TabsTrigger>
            <TabsTrigger value="insights" style={{ fontSize: `${15 * textScalar}px` }}>{t("er.detailsDialog.insights")}</TabsTrigger>
          </TabsList>
          ...
          >
            {summary || <p>{t("procurement.detailsDialog.noSummary")}</p>}
          </TabsContent>
          ...
          <TabsContent
            ...
            className="text-sm space-y-2 scale-[0.95] sm:scale-100 overflow-y-auto"
          >
            {dataAndFilters || <p>{t("procurement.detailsDialog.noData")}</p>}
          </TabsContent>
          ...
          >
            {insights || <p>{t("procurement.detailsDialog.noInsights")}</p>}
          </TabsContent>
        </Tabs>
      </DialogContent>
    </Dialog>
